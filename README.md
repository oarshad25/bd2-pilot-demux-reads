...
(specified by `reads_dirs` in config), merges demultiplexed reads per barcode
and places the final reads for each barcode into seperate subdirectories for each barcode within
the specified output directory and computes statistics on demuxed reads (no. of reads, N50 etc.)
...
4. Move fastq's for each barcode and unclassified reads into own subdirectories within the dorado demux output directory
5. Merge fastq's per barcode and unclassified reads and place them in specified output directory (in config) with seperate subdirectories for each barcode.
6. Compute statistics on each barcode fastq.

If multiple sequencing run directories are provided in `reads_dirs`, one fastq is output per sequencing run in each barcode
subdirectory after step 4:

``` bash
dorado_demux_out_dir/
    ├── barcode01/
...
which are merged per barcode to create the output directory:
...
1. *Read directory/directories:* Path(s) to directory (directories) of attempted demultiplexed reads by the sequencer using standard dorado demux configuration.
...
4. *output_reads:* Output directory of demultiplexed reads with subdirectories for
   each barcode containing the demultiplexed fastq (demultiplexed fastqs per sequencing input directory specified in `reads_dirs` merged per barcode) for the
   corresponding barcode
5. *seqkit_stats:* Statistics table of demultiplexed read fastqs.
